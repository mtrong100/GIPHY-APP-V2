...
const itemLoading = 8;
const GifSlider = ({ query, limit = "25" }) => {
  const [url, setUrl] = useState(GIPHY_API.getGifs("trending", limit));

  /* Check params to switch API fetching */
  useEffect(() => {
    if ((query, limit)) {
      setUrl(GIPHY_API.getSearchGif(query, limit));
    } else {
      setUrl(GIPHY_API.getGifs("trending", limit));
    }
  }, [query]);
